...
pwsh -File scripts\Run-Agent.ps1 -Model gpt-5.3-codex -Role implementer -Yolo -Prompt "..."
```

**Claude sub-agent** использует `--dangerously-skip-permissions`:
```powershell
# Claude sub-agent без диалогов подтверждения
pwsh -File scripts\Run-Agent.ps1 -Model claude-sonnet-4-6 -Role implementation-planner -Prompt "..."
```
...
Текущая цепочка в `.claude/dispatcher.config.json`:
```json
{ "chain": [
...
В `.claude/agents/` определены профили субагентов для Claude Code:
