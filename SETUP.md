...
## ФОРМАТЫ УСТАНОВКИ (Local vs Global)

В системе предусмотрено **2 формата работы**:

1. **Local (В рамках одного проекта)**  
   Папка `.claude` и `scripts` лежат прямо в корне твоего текущего проекта. `dispatcher.config.json` и файлы ролей настраиваются индивидуально только для этого проекта.
2. **Global (Глобальная установка)**  
   Копирование содержимого в глобальную профильную папку (например, `C:\Users\User\.claude`). Это делается вручную или через скрипт копирования. После этого оркестратор и цепочки агентов становятся доступны на уровне **всех проектов** в системе. (Локальный конфиг проекта при этом имеет приоритет над глобальным).

---

## 3 ВАРИАНТА ИСПОЛЬЗОВАНИЯ (SETUPS)

Существует 3 разных способа пробросить эти инструкции в агента (Claude Code):

1. **Работа через CLAUDE.md (Custom Instructions)**  
   Основной и самый надежный способ. Мы прописываем подробный гайд в файл `CLAUDE.md`, в котором заставляем Claude выступать Оркестратором/Планировщиком (см. `CLAUDE_EXAMPLE.md`). Этот файл автоматически считывается Claude при старте.
2. **Работа через Хуки (Hooks)**  
   Автоматизация процесса "на лету" без засорения контекста. Используются `on-prompt.ps1` (инжект требований перед ответом) и `pre-bash.ps1` (перехват команд). Хуки настраиваются в файле `settings.json`.
3. **Работа через Скиллы (Skills)**  
   Оформление скриптов делегирования (таких как `Run-Agent.ps1` или `Invoke-Chain.ps1`) в виде нативных скиллов (многофункциональных инструментов) для Claude, чтобы агент воспринимал их как часть своего встроенного тулсета, а не просто как внешние Bash-команды.

---

## КАК ЭТО РАБОТАЕТ (на примере хуков): один терминал, никаких доп. окон
...
**Ключевое:** всё в одном терминале Claude Code. `gemini` и `codex` — это просто CLI-программы, запускаемые как subprocess через `Bash` tool. Никаких дополнительных окон не нужно.
